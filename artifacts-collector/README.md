...
- must-gather: used as a baseline to evaluate cluster information, and etcd performance during the conformance tests. Must-gather is the first data collected - runs before performance tests, so we report won't be impacted with non-e2e workloads which could impact in the final results (non-regular workload) 
