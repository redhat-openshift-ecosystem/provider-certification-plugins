...
save_results() {
    ...
    os_log_info "[runner] Saving results triggered. Slowing down..."
    sleep 5

    pushd ${results_dir};

    # JUnit
    os_log_info "[runner] Looking for junit result files..."
    ...
    if [[ -z "${JUNIT_OUTPUT}" ]]; then
        local res_file="junit_fake_e2e_$(date +%Y%m%d-%H%M%S).xml"
        os_log_info "[runner] creating empty junit result file [${res_file}]"
        cat << EOF > "${res_file}"
...
EOF
    ...
    os_log_info "[runner] adjusting permissions to results."
    chmod 644 ${JUNIT_OUTPUT};

    os_log_info "[runner] telling sonobuoy worker the result file"
...
os_log_info "[runner] starting executor..."
...
os_log_info "Plugin runner finished. Result[$?]";
